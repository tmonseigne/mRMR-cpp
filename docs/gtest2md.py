...
def get_xml_attribute(convert_func, xml_node, attribute_name, default_value, print_warning=True):
	if not attribute_name in xml_node.attrib and print_warning:
		print('Warning: Attribute {!r} was not found inside xml node {!s}[{!r}]. Set it to its default value {!r}.'.format(attribute_name, xml_node.tag, xml_node.attrib, default_value))
...
def generate_summary(xml_testsuites_node):
	...
	print(str(total_test) + ' tests, in ' + total_execution_time + ' sec (' + str(total_success) + ' success).')
...
def generate_tests_result(xml_testsuites_node):
	...
	for xml_testsuite_node in xml_testsuite_nodes:
		...
		print('\tTest suite : ' + testsuite_name + ', ' + str(testsuite_test) + ' tests, in ' + testsuite_execution_time + ' sec (' + str(testsuite_success) + ' success).')

		# Generate the table for the testsuite summary.
